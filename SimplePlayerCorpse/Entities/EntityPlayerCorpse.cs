...
public class EntityPlayerCorpse : EntityAgent
{
    ...
    public override void OnGameTick(float dt)
    {
        ...
        if (Api is ICoreClientAPI capi)
        {
            if (OwnerUID == capi.World.Player.PlayerUID && Api.World.Rand.NextDouble() < 0.3)
            {
                ...
                {
                    MinPos = Pos.XYZ,
                    Color = GetRandomParticleColor(Api.World.Rand),
                    MinSize = 0.2f,
                    MaxSize = 0.3f,
                    ...
                    LightEmission = ColorUtil.ColorFromRgba(255, 255, 255, 255)
                });
            }
        }
    }
            ...
            if (byPlayer != null)
            {
                ...
                else
                {
                    if (Api.Side == EnumAppSide.Server)
                    {
                        ...
                        {
                            var format = "{0} at {1} is empty and will be removed immediately, id {3}";
                            Message(format, byPlayer);
                            Die(EnumDespawnReason.Removed);
                        }
                        else
                        {
                            Collect(byPlayer);
                            DeathContentManager.RemoveDeathPoint(entityPlayer, this);
                        }
                    }
    ...
    private void Collect(IPlayer byPlayer)
    {
        ...
        var format = "{0} at {1} can be collected by {2}, id {3}";
        Message(format, byPlayer);
    ...
    private void Message(string format, IPlayer byPlayer)
    {
        var corpseName = GetName();
        var position = Utils.MakeRelativePos(Pos.XYZ, Api.World.DefaultSpawnPosition.XYZ);
        var playerName = byPlayer.PlayerName;
        var msg = string.Format(format, corpseName, position, playerName, EntityId);
        ModLogger.Notification(msg);
    }

    private static int GetRandomParticleColor(Random rand)
    {
        int a = 255;
        ...
        /* particles use bgra instead of rgba */
        return ColorUtil.ColorFromRgba(b, g, r, a);
    }
}